...
class Lista:
    ...
    def add_inicio(self, elemento):
        if self.validar_tipo(elemento):
            ...
            if self.vazia(): # self.inicio is None:
                ...
                self.fim = no
                self.meio = no
            elif (self.__tamanho % 2) == 0:
                no.proximo = self.inicio
                self.inicio.anterior = no
                no.anterior = None
                self.inicio = no
            else:
                metade = self.__tamanho / 2
                no.proximo = self.inicio
                self.inicio.anterior = no
                no.anterior = None
                self.inicio = no
    ...
    def listar_consultas_cpf(self, cpf):
        lista=[]
        perc = self.inicio
        while perc.proximo is not None:
            if perc.elemento.cpf_paciente == cpf:
                lista.append(perc.elemento)
            perc = perc.proximo
        return lista

    def listar_consultas_crm(self, crm):
        lista = []
        perc = self.inicio
        while perc.proximo is not None:
            if perc.elemento.crm == crm:
                lista.append(perc.elemento)
            perc = perc.proximo
        return lista
